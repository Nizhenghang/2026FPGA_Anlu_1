...
板载 Type-C 串口屏经 UART 成为主控制面，替代 `key1`/`key2`/`key3` 与 `SW1`~`SW4` 的全部功能，并扩展出直接选图 / 直接设亮度 / 直接选特效，以及**实体控制根本没有的三项——图像点运算 `FILT`、字幕立体字 `FONT` 与音频源 `MUSC`**。`MODE` 也是「超出」而不只是「替代」：转场码空间这一轮从 3 位扩到 4 位，`8`~`E` 七个模式在硬件上无法从拨码到达（物理项是 `{1'b0, ~sw_v1[2:0]}`），只能由屏幕选中。**实体按键与拨码完整保留为兜底**：屏幕没接、没上电或故障时，板上交互与改动前逐位一致。合并策略是 last-writer-wins，两端互为退路；`FILT`/`FONT`/`MUSC` 没有物理对端，它们的退路是复位值（直通 / 平面 / 测试音），详见下面「兜底语义」。
...
- **方向：目前只有「屏 → FPGA」一个方向。** `uart_tx`（D12）在 Stage 1 里被硬接成空闲高（`assign uart_tx = 1'b1;`），FPGA **一个字节都不回发**——没有 ACK、没有错误码、没有状态回传。这直接决定屏幕 UI 的写法：所有状态只能由屏幕自己记账，唯一的权威回显是 HDMI 画面上的 OSD 面板（第 6 节，第 3 行 `HDMI AUDIO TONE/MUSIC`、第 5 行 `FX:` / `FONT:` 都是实时状态行）。发错命令的表现就是「什么都没发生」，屏幕上不会有任何提示。状态回传属于 Stage 2，见本节末。
...
> **交付件**：本节可整节渲染成 Word 交给屏幕工程的人——`python tools/gen_uart_protocol_docx.py` 生成 `doc/串口屏控制协议_淘晶驰.docx`（A4 横向，6 张表）。那份 docx 是**纯生成物**：表格与文字逐字解析自本 README，脚本里没有任何硬编码的协议副本，所以它不可能与本节漂移；协议改了只改 README，然后重跑脚本，**不要直接编辑 docx**。

**三条照抄时的坑**：`print` 里的空格必须是 ASCII `0x20`，不能是全角空格或 Tab；`print` **不会替你转大小写**，关键字与 `A`-`F` 都必须写成大写（小写 `a`-`f` 落在 `0x61`~`0x66`，两个合法区间都不含，一律拒收）；`MODE`/`FILT` 是**十六进制单字符**，想表达 10 就写 `A`，**不存在两位数**——`MODE 10` 会让 `clen=7` 而整帧被丢弃。
...
8. 图像算法与立体字的上板验收（`FILT` / `FONT` 是屏幕独有、**没有实体兜底**，所以必须先接上串口屏）：
